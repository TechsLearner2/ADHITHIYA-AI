"""
Drop-in ADHITHIYA plugin template.

Copy this file, rename it (no leading underscore), fill in PLUGIN and run().
No other file needs to change — ADHITHIYA discovers this automatically at startup.
"""
...
def run(parameters: dict, player=None, session_memory=None) -> str:
    ...
    if player:
        try:
            player.write_log(f"ADHITHIYA: {result_text}")
        except Exception:
            pass
    return result_text
