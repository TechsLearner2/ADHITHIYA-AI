"""
ADHITHIYA plugin — Calorie Counter (webcam vision).
...
_SCAN_COLOR        = (255, 190, 40)    # ADHITHIYA cyan-blue (BGR)
...
def run(parameters: dict, player=None, session_memory=None) -> str:
    ...
    try:
        if stream_sig:
            ...
            view_open = True
        _log("ADHITHIYA: Nutrition scan started.")
    ...
    try:
        ...
        if view_open:
            stream_sig.emit(False)  # back to the ADHITHIYA HUD
    ...
    if data.get("food"):
        ...
        if player and panel:
            try:
                ...
                pass
        _log(f"ADHITHIYA: Nutrition scan complete — {data.get('food')}"
