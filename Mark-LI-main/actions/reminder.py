...
try:
    ...
    notification.notify(title="ADHITHIYA Reminder", message=message, timeout=15)
    notified = True
except Exception:
    pass
...
if not notified:
    try:
        ...
        ToastNotifier().show_toast("ADHITHIYA Reminder", message, duration=15, threaded=False)
        notified = True
    except Exception:
        pass
...
if not notified:
    try:
        import subprocess
        script = 'display notification "{{}}" with title "ADHITHIYA Reminder"'.format(
            message.replace('"', '')
        )
...
"""
    ...
    script_body = f"""# Auto-generated by ADHITHIYA reminder — do not edit
...
def _schedule_windows(target_dt: datetime, task_name: str,
    ...
    xml_content = (
        ...
        '  <RegistrationInfo><Description>ADHITHIYA Reminder</Description></RegistrationInfo>\n'
...
def reminder(
    ...
    task_name  = f"ADHITHIYAReminder_{target_dt.strftime('%Y%m%d_%H%M%S')}"
