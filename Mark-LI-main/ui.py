...
from PyQt6.QtGui import (
    ...
    QPen, QPixmap, QRadialGradient, QShortcut, QIcon, QAction,
)
...
class AudioDiagnosticsPanel(QWidget):
    """Non-modal audio diagnostics and local voice-profile controls.

    Audio is captured only in worker threads and is discarded after each test
    or enrollment.  The only persistent voice artifact is the embedding
    written by ``core.voice_gate``.
    """

    level_changed = pyqtSignal(int)
    result_changed = pyqtSignal(str)
    voice_refresh = pyqtSignal()
    ...
    def __init__(self, parent=None):
        ...
        self.setObjectName("AudioDiagnosticsPanel")
        ...
        self.setStyleSheet(f"""
            QWidget#AudioDiagnosticsPanel {{
                background: {C.DARK}; color: {C.TEXT};
                border: 1px solid {C.BORDER_B}; border-radius: 5px;
            }}
            QLabel {{ color: {C.TEXT_MED}; background: transparent; }}
            ...
            QPushButton:hover {{ border-color: {C.PRI}; background: {C.PRI_GHO}; }}
        """)
        lay = QVBoxLayout(self)
        lay.setContentsMargins(14, 12, 14, 12)
        lay.setSpacing(8)
        heading = QLabel("◈ AUDIO DIAGNOSTICS")
        heading.setFont(QFont("Courier New", 10, QFont.Weight.Bold))
        ...
        lay.addWidget(QLabel("INPUT LEVEL (memory-only monitor)"))
        lay.addWidget(self._level)
        row = QHBoxLayout()
        self._monitor_btn = QPushButton("START LEVEL")
        self._monitor_btn.clicked.connect(self._toggle_level)
        self._mic_btn = QPushButton("TEST MICROPHONE")
        self._mic_btn.clicked.connect(self._microphone_test)
        self._speaker_btn = QPushButton("TEST SPEAKER")
        self._speaker_btn.clicked.connect(self._speaker_test)
        for button in (self._monitor_btn, self._mic_btn, self._speaker_btn):
            row.addWidget(button)
        lay.addLayout(row)
        lay.addWidget(QLabel("LOCAL VOICE PROFILE"))
        voice_row = QHBoxLayout()
        self._enroll_btn = QPushButton("ENROLL (5s)")
        self._enroll_btn.clicked.connect(self._enroll)
        self._clear_voice_btn = QPushButton("CLEAR")
        self._clear_voice_btn.clicked.connect(self._clear_voice)
        voice_row.addWidget(self._enroll_btn)
        ...
        self._result = QLabel("No test run.")
        self._result.setWordWrap(True)
        lay.addWidget(self._result)
        close = QPushButton("CLOSE")
        close.clicked.connect(self._close)
        lay.addWidget(close)
        self.level_changed.connect(self._level.setValue)
        self.result_changed.connect(self._result.setText)
        self.voice_refresh.connect(self._refresh_voice_status)
        self.busy_changed.connect(self._set_enroll_enabled)
    ...
    def _populate_devices(self):
        ...
        try:
            ...
            configured = None
            try:
                from memory.config_manager import get_audio_input_device
                configured = get_audio_input_device()
            except Exception:
                pass
            selected = -1
            for index, info in enumerate(devices):
                ...
                if configured is not None and (configured == index or configured == name):
                    selected = self._device.count() - 1
            if selected >= 0:
                self._device.setCurrentIndex(selected)
            self._device.currentIndexChanged.connect(self._device_changed)
        except Exception as exc:
            self._device.addItem("Default input device", None)
            self._result.setText(f"Audio devices unavailable: {exc}")

    def _device_changed(self):
        self.device_changed.emit(self.selected_device)
        try:
            from memory.config_manager import save_audio_input_device
            save_audio_input_device(self.selected_device)
        except Exception:
            pass
    ...
    def _toggle_level(self):
        if self._stream_thread and self._stream_thread.is_alive():
            ...
            self._monitor_btn.setText("START LEVEL")
            return
        self._stream_stop.clear()
        self._monitor_btn.setText("STOP LEVEL")
        device = self.selected_device
        self._stream_thread = threading.Thread(target=self._level_loop,
                                                args=(device,), daemon=True)
        self._stream_thread.start()

    def _level_loop(self, device):
        try:
            import numpy as np
            import sounddevice as sd
            def callback(indata, _frames, _time, _status):
                ...
                self.level_changed.emit(max(0, min(100, int(rms * 260))))
            with sd.InputStream(device=device, samplerate=16000,
                                channels=1, dtype="float32", blocksize=1024,
                                callback=callback):
                while not self._stream_stop.wait(0.1):
                    pass
        except Exception as exc:
            self.result_changed.emit(f"Input level unavailable: {exc}")
        finally:
            self.level_changed.emit(0)

    def _microphone_test(self):
        if self._busy:
            return
        self._busy = True
        self._result.setText("Recording a 2 second microphone test…")
        threading.Thread(target=self._record_test, args=(self.selected_device,), daemon=True).start()

    def _record_test(self, device):
        try:
            import sounddevice as sd
            import numpy as np
            recording = sd.rec(2 * 16000, samplerate=16000, channels=1,
                               dtype="int16", device=device)
            sd.wait()
            level = float(np.sqrt(np.mean(np.square(np.asarray(recording, dtype=np.float32)))) / 32768)
            self.result_changed.emit(f"Microphone OK — peak level {min(100, int(level * 260))}%. Audio discarded.")
            del recording
        except Exception as exc:
            self.result_changed.emit(f"Microphone test failed: {exc}")
        finally:
            self._busy = False
    ...
    def _speaker_test(self):
        if self._busy:
            return
        self._busy = True
        threading.Thread(target=self._play_test, daemon=True).start()
    ...
    def _play_test(self):
        try:
            ...
            sd.wait()
            self.result_changed.emit("Speaker test played. No audio was saved.")
            del tone, t
        except Exception as exc:
            self.result_changed.emit(f"Speaker test failed: {exc}")
        finally:
            self._busy = False

    def _refresh_voice_status(self):
        try:
            from memory.config_manager import get_voice_profile_path
            from core.voice_gate import voice_profile_status
            path = get_voice_profile_path()
            status = voice_profile_status(path)
            self._voice_status.setText(
                f"Status: {status}  ·  {path}\n"
                "Only a local speaker embedding is retained; raw enrollment audio is never stored."
            )
        except Exception as exc:
            self._voice_status.setText(f"Voice profile status unavailable: {exc}")
    ...
    def _enroll(self):
        ...
        self._result.setText("Speak naturally for 5 seconds… audio remains in memory only.")
        threading.Thread(target=self._enroll_worker, args=(self.selected_device,), daemon=True).start()

    def _enroll_worker(self, device):
        recording = None
        try:
            import sounddevice as sd
            import numpy as np
            from memory.config_manager import get_voice_profile_path
            from core.voice_gate import enroll_voice_profile
            recording = sd.rec(5 * 16000, samplerate=16000, channels=1,
                               dtype="int16", device=device)
            sd.wait()
            result = enroll_voice_profile(np.asarray(recording).reshape(-1).tobytes(),
                                          profile_path=get_voice_profile_path(), replace=True)
            self.result_changed.emit(f"Voice profile enrolled ({result['model']}). Raw audio discarded.")
            self.voice_refresh.emit()
        except Exception as exc:
            self.result_changed.emit(f"Enrollment failed: {exc}")
        finally:
            recording = None
            self._busy = False
            self.busy_changed.emit(False)

    def _set_enroll_enabled(self, busy: bool):
        self._enroll_btn.setEnabled(not busy)

    def _clear_voice(self):
        try:
            from memory.config_manager import get_voice_profile_path
            from core.voice_gate import clear_voice_profile
            clear_voice_profile(get_voice_profile_path())
            self._result.setText("Local voice profile cleared. Raw audio was not retained.")
            self._refresh_voice_status()
        except Exception as exc:
            self._result.setText(f"Could not clear voice profile: {exc}")
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, face_path: str):
        ...
        self._audio_panel: AudioDiagnosticsPanel | None = None
        self._quitting = False
        self.on_agent_approve = None
        self.on_agent_reject = None
        self.audio_input_device = _cfg.get("audio_input_device")
        ...
        self._audio_panel = AudioDiagnosticsPanel(self.centralWidget())
        self._audio_panel.hide()
        self._audio_panel.closed.connect(self._audio_panel_closed)
        self._audio_panel.device_changed.connect(self._set_audio_input_device)
        self._setup_tray()
        ...
        try:
            ...
            # ── macOS — proper .app bundle (no Terminal window) ───────────────
                ...
                (app / "Contents" / "Info.plist").write_text(
                    ...
                    '  <key>CFBundleExecutable</key><string>ADHITHIYA</string>\n'
                    '  <key>CFBundleIdentifier</key>'
                    '<string>com.adhithiya.assistant</string>\n'
                    '  <key>CFBundleName</key><string>ADHITHIYA</string>\n'
            ...
            # ── Linux — .desktop file (Terminal=false, no console) ────────────
                ...
                desk = desktop / "ADHITHIYA.desktop"
                desk.write_text(
                    "[Desktop Entry]\n"
                    "Name=ADHITHIYA\n"
    ...
    def resizeEvent(self, event):
        ...
        if self._audio_panel and self._audio_panel.isVisible():
            aw, ah = self._audio_panel.width(), self._audio_panel.height()
            self._audio_panel.setGeometry(
                (cw.width() - aw) // 2, (cw.height() - ah) // 2, aw, ah
            )
    ...
    def _set_audio_input_device(self, device):
        self.audio_input_device = device

    def _audio_panel_closed(self):
        if self._audio_panel:
            self._audio_panel.hide()

    def _open_audio_diagnostics(self):
        if not self._audio_panel:
            return
        cw = self.centralWidget()
        aw, ah = self._audio_panel.width(), self._audio_panel.height()
        self._audio_panel.setGeometry((cw.width() - aw) // 2,
                                      (cw.height() - ah) // 2, aw, ah)
    ...
    def _setup_tray(self):
        ...
        self._tray_mute = QAction("Mute microphone", self)
        self._tray_mute.setCheckable(True)
        self._tray_mute.toggled.connect(self._tray_mute_changed)
        menu.addAction(self._tray_mute)
        menu.addSeparator()
        status = QAction(
            f"Launch at login: {'ON' if self._check_autostart() else 'OFF'}", self
        )
        status.setEnabled(False)
        self._tray_login_status = status
        menu.addAction(status)
        menu.addSeparator()
        quit_action = QAction("Quit ADHITHIYA", self)
    ...
    def _toggle_window(self):
        if self.isVisible() and not self.isMinimized():
            self.hide()
            if self._tray:
                self._tray_toggle.setText("Show window")
        else:
            self.showNormal()
            self.raise_()
            self.activateWindow()
            if self._tray:
                self._tray_toggle.setText("Hide window")

    def _tray_mute_changed(self, checked: bool):
        if checked != self._muted:
            self._toggle_mute()
    ...
    def _build_quick_drawer(self) -> QWidget:
        ...
        self._agent_state_lbl.setStyleSheet(f"color: {C.TEXT_DIM}; background: transparent;")
        lay.addWidget(self._agent_state_lbl)
        self._agent_progress_display = QTextEdit()
        self._agent_progress_display.setReadOnly(True)
        self._agent_progress_display.setFixedHeight(78)
        self._agent_progress_display.setFont(QFont("Courier New", 7))
        self._agent_progress_display.setStyleSheet(
            f"QTextEdit {{ background: {C.PANEL}; color: {C.TEXT}; "
            f"border: 1px solid {C.BORDER}; }}"
        )
        lay.addWidget(self._agent_progress_display)
        agent_row = QHBoxLayout()
        self._agent_approve_btn = QPushButton("APPROVE")
        self._agent_reject_btn = QPushButton("REJECT")
        for button in (self._agent_approve_btn, self._agent_reject_btn):
            button.setFixedHeight(23)
            button.setFont(QFont("Courier New", 7, QFont.Weight.Bold))
            button.setCursor(Qt.CursorShape.PointingHandCursor)
        self._agent_approve_btn.clicked.connect(self._approve_agent)
        self._agent_reject_btn.clicked.connect(self._reject_agent)
        self._agent_approve_btn.setEnabled(False)
        self._agent_reject_btn.setEnabled(False)
        agent_row.addWidget(self._agent_approve_btn)
        agent_row.addWidget(self._agent_reject_btn)
    ...
    def _approve_agent(self):
        if self.on_agent_approve:
            self.on_agent_approve()

    def _reject_agent(self):
        if self.on_agent_reject:
            self.on_agent_reject()

    def _append_agent_progress(self, message: str):
        self._agent_progress_display.append(message[:1200])

    def _show_agent_result(self, result):
        state = getattr(result, "state", None)
        state_text = getattr(state, "value", str(state or "idle")).upper()
        message = getattr(result, "message", str(result))
        self._agent_state_lbl.setText(f"{state_text}: {message[:500]}")
        self._append_agent_progress(message)
        pending = state_text == "AWAITING_APPROVAL"
        self._agent_approve_btn.setEnabled(pending)
        self._agent_reject_btn.setEnabled(pending)
        if pending:
    ...
    def _check_autostart(self) -> bool:
        ...
        try:
            if _OS == "Windows":
                ...
                return any((launch_dir / name).exists() for name in (
    ...
    def _toggle_autostart(self):
        ...
        try:
            ...
            if _OS == "Windows":
                ...
                plist = plist_dir / "com.adhithiya.assistant.plist"
                if currently_on:
                    ...
                    plist.write_text(
                        ...
                        '"http://www.apple.com/DTDs/PropertyList-1.0.dtd">\n'
                        '<plist version="1.0"><dict>\n'
                        '  <key>Label</key><string>com.adhithiya.assistant</string>\n'
            ...
            if getattr(self, "_tray_login_status", None):
                self._tray_login_status.setText(f"Launch at login: {'ON' if enabled else 'OFF'}")
            self._log.append_log(
    ...
    def _toggle_mute(self):
        ...
        self._style_mute_btn()
        if self._tray:
            self._tray_mute.blockSignals(True)
            self._tray_mute.setChecked(self._muted)
            self._tray_mute.setText("Unmute microphone" if self._muted else "Mute microphone")
            self._tray_mute.blockSignals(False)
        if self._muted:
...
class AdhithiyaUI:
    ...
    @property
    def on_agent_approve(self):
        return self._win.on_agent_approve

    @on_agent_approve.setter
    def on_agent_approve(self, cb):
        self._win.on_agent_approve = cb

    @property
    def on_agent_reject(self):
        return self._win.on_agent_reject

    @on_agent_reject.setter
    def on_agent_reject(self, cb):
        self._win.on_agent_reject = cb

    @property
    def audio_input_device(self):
        return self._win.audio_input_device

    def show_agent_result(self, result) -> None:
        self._win._agent_result_sig.emit(result)

    def agent_progress(self, message: str) -> None:
        self._win._agent_progress_sig.emit(str(message))
